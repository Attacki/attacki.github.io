...
![Mvvm demo]( ../../assets/images/mvvm_demo.gif)

```html
<div id="app">
    <div>{{message.greet}}</div>
    <input type="text" v-model='message.greet'></br></br></br>
    <span>
        {{message.tip}}
    </span></br>
    <input type="text" v-model='message.greet'></br>
    {{message.tip}}</br>
...
## 订阅者
订阅者简单来说，就是将观察者都放在一个数组里，并提供添加及调用观察者update的方法
